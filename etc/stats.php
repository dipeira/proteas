...
<html>
    ...
    echo "<script>";
    echo "const formatPieTooltip = function(context) {";
    echo "  const dataset = context.chart && context.chart.data && context.chart.data.datasets ? context.chart.data.datasets[context.datasetIndex] : null;";
    echo "  const dataArray = dataset && Array.isArray(dataset.data) ? dataset.data : [];";
    echo "  let rawValue = 0;";
    echo "  if (typeof context.raw !== 'undefined') {";
    echo "    rawValue = context.raw;";
    echo "  } else if (typeof context.parsed !== 'undefined') {";
    echo "    rawValue = context.parsed;";
    echo "  } else if (typeof dataArray[context.dataIndex] !== 'undefined') {";
    echo "    rawValue = dataArray[context.dataIndex];";
    echo "  }";
    echo "  const value = Number(rawValue) || 0;";
    echo "  const total = dataArray.reduce(function(sum, item) {";
    echo "    const numericValue = Number(item);";
    echo "    return sum + (isNaN(numericValue) ? 0 : numericValue);";
    echo "  }, 0);";
    echo "  const percentage = total > 0 ? ((value / total) * 100).toFixed(1) : '0.0';";
    echo "  const label = context.label || (context.dataset && context.dataset.label) || '';";
    ...
    if (count($monimoi_labels) > 0) {
        ...
        echo "            return formatPieTooltip(context);";
        echo "          }";
        echo "        }";
        echo "      }";
        echo "    }";
