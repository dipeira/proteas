...
<?php       
    ...
    if($_POST['type'] == 1 || $_POST['type'] == 7)
    {
        ...
        // kratikoy or ESPA 
        if ($kratikoy)
            $query = "SELECT e.id,e.name,e.surname,e.patrwnymo,e.klados,p.name as praksi,p.ya,p.ada,p.apofasi,p.type,e.hm_anal,e.metakinhsh,e.afm,e.type as typos from ektaktoi e JOIN praxi p ON e.praxi = p.id WHERE e.type IN (1,2) AND p.type ='ΚΡΑΤ'";
        else
            $query = "SELECT e.id,e.name,e.surname,e.patrwnymo,e.klados,p.name as praksi,p.ya,p.ada,p.apofasi,p.type,e.hm_anal,e.metakinhsh,e.afm,e.type as typos from ektaktoi e JOIN praxi p ON e.praxi = p.id WHERE e.type IN (1,3,4,5,6) AND p.type !='ΚΡΑΤ'";
        ...
        // ***********************
        {
            ...
            // ?????
            // if (strpos($praksi,'ΕΕΠ') !== false)
            //     $eepebp = 1;
            // else if (strpos($praksi,'ΕΒΠ') !== false)
            //     $eepebp = 2;
