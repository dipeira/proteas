...
<body>
    ...
    <center>
        ...
        $query = "SELECT * FROM adeia ad RIGHT JOIN employee emp ON ad.emp_id = emp.id WHERE ((start<'$today' AND finish>'$today') OR status=3) 
            AND sx_organikhs NOT IN ($allo_pyspe, $allo_pysde) 
        ...
        if ($num) {
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                $status = $row['status'];
                $am = $row['am'];
                $sx_organikhs = getSchool($row['sx_organikhs'], $mysqlconnection);
                ...
                if ($flag) {
                    ...
                    //    continue;
                    echo "<tr>";
                    echo "<td>$am</td><td><a href=\"employee.php?id=$id&op=view\">" . $surname . "</a></td><td>" . $name . "</td><td>" . $klados . "</td><td>$typewrd</td><td>$sx_organikhs</td><td><a href='adeia.php?adeia=$adeia_id&op=view'>$ret</a></td><td>$comments</td>\n";
                    echo "</tr>";
                }
