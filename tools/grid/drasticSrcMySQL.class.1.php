...
class drasticSrcMySQL {
	...
	function __construct($server, $user, $pw, $db, $table, $options = null) {
		...
		// Do the sorting:
		if (isset($_REQUEST["sortcol"])) $this->sortcol = mysqli_real_escape_string($_REQUEST["sortcol"]);
		if (isset($_REQUEST["sort"])) $this->sort = mysqli_real_escape_string($_REQUEST["sort"]);
		...
		if (isset($_REQUEST["op"])) 	$op		= mysqli_real_escape_string($_REQUEST["op"]); else $op = "";		
		if (isset($_REQUEST["id"])) 	$id		= mysqli_real_escape_string($_REQUEST["id"]);
		if (isset($_REQUEST["col"])) 	$col	= mysqli_real_escape_string($_REQUEST["col"]);
		if (isset($_REQUEST["value"])) 	$value	= mysqli_real_escape_string($_REQUEST["value"]);
	...
	private function view(){
		...
		if (isset($_REQUEST["cols"])) $cols = explode(",", mysqli_real_escape_string($_REQUEST["cols"]), $this->num_fields);
		if (isset($_REQUEST["id"])){
			...
			if (isset($_REQUEST["start"])) $start = mysqli_real_escape_string($_REQUEST["start"]); else $start = 0;
			if (isset($_REQUEST["end"])) $end = mysqli_real_escape_string($_REQUEST["end"]); else $end= $this->num_rows;
			if ($start < 0) $start=0;
		...
		if (isset($_REQUEST["sns"]) && strlen($_REQUEST["sns"]) > 0) {
			$sumnames = explode(",", mysqli_real_escape_string($_REQUEST["sns"]));
	...
	private function view_bar(){
		...
		$colname = mysqli_real_escape_string($_REQUEST["colname"]);
		$w = mysqli_real_escape_string($_REQUEST["w"]);
		$h = mysqli_real_escape_string($_REQUEST["h"]);
