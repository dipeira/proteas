...
<?php
		...
		$num=mysql_num_rows($result);
                ...
                echo "<thead><tr><th>id</th><th>surname, name</th><th>hm_dior</th><th>proyp_old</th><th>proyp</th><th>met_did</th><th>anatr_excel</th><th>ALLAGH</th><th>diafora</th></tr></thead>";
                echo "<tbody>";
	while ($i<$num)	
	{
		...
		$proyp_old = mysql_result($result, $i, "proyp");
		$met_did = mysql_result($result, $i, "met_did");
                $anatr = mysql_result($result, $i, "anatr_excel");
		$ymd = days2date($anatr);
		...
		//if ((strcmp($_GET['id'],'fixall')==0) && ($proyp != $proyp_old))
                if ((strcmp($_GET['id'],'fixall')==0) && ($proyp != $proyp_old))
		{
			$query = "UPDATE employee SET proyp='$proyp' WHERE ID=$id";
			echo "&nbsp;&nbsp;$query";
                        echo "<br>";
